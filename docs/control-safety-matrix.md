...
| Stationary-only settings writes | `SetPedalMode` (typed; Veteran `SETh`/`SETm`/`SETs`, Begode `h`/`f`/`s`) | `SupportsSettingsWrites` plus `StationarySettingsWriteSession` | A `StationarySettingsPolicy` may arm only from `Parked` or `Standing`; the session then requires matching model/capability, rejects expiry, and clears the arm on link-down | Encoder and refusal paths tested; no model is currently registered for live writes |
