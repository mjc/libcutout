...
| Stationary-only settings writes | `ResetTripMeter`, `SetAeroTiltbackSpeed`, `SetAeroPwmPercent`, `SetAeroAlarmSpeed`, `SetAeroAngleAdjustment`, `SetAeroHighBeam`, `SetPedalMode`, `SetRollAngle`, `SetSpeedAlarmMode`, `SetBegodeMaxSpeed`, `SetBegodeBeeperVolume`, `SetBegodeLedMode`, `SetAccelerationAssist` | `SupportsSettingsWrites` plus `StationarySettingsWriteSession` | A `StationarySettingsPolicy` may arm from `Parked` or `Standing` (or a model-specific bounded low-speed window); the session requires matching model/capability, seeds its monotonic clock from the arm, rejects expiry, and clears the arm on link-down | NOSFET Aero is registered for the explicit Rust `aero-write` path. The simulator exercises every typed encoder; the mobile wrapper exposes only hardware-validated headlight/high-beam writes. TLT/ALM/pedal have typed readback mappings but remain unverified until the corresponding capture and NF2557 evidence is recorded; PWT/ANG/trip reset also lack live readback fields. |
