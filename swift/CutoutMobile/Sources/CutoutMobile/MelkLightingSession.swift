...
import Foundation
    ...
    extension MelkLightingPeripheralState {
        ...
        public var resetsRestoreEligibility: Bool {
            switch self {
        ...
        @unchecked Sendable
    {
        ...
        public func markLastCommandUnconfirmed() {
            onQueue {
                core.markLastCommandUnconfirmed()
                syncCore()
            }
        }
        ...
        public func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
            onQueue {
                ...
                core.handle(
                    event: .connected(name: peripheral.name, platformIdentifier: peripheral.identifier.uuidString))
                syncCore()
            }
        }
        ...
        public func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
            onQueue {
                ...
                let services =
                    peripheral.services?.compactMap { service in
                        MobileBluetoothUuid(coreBluetoothUuid: service.uuid)
                    } ?? []
                core.handle(
                    event: .servicesDiscovered(serviceUuids: services, error: error.map(String.init(describing:))))
                syncCore()
            }
        }
                    ...
                    service.characteristics?.compactMap { characteristic in
                        MobileBluetoothUuid(coreBluetoothUuid: characteristic.uuid).map {
                            MobileMelkLightingCharacteristicEvidenceDto(
                                ...
                                notifyOrIndicate: characteristic.properties.contains(.notify)
        ...
        private func syncCore() {
            ...
            for candidate in core.drainCandidates() {
                onCandidate?(
                    MelkLightingPeripheralCandidate(
                        name: candidate.name,
            ...
            for action in actions {
                ...
                case let .connect(identifier):
                    guard
                        let candidate = discoveredPeripherals[identifier]
                            ?? central.retrievePeripherals(withIdentifiers: [UUID(uuidString: identifier) ?? UUID()])
                            .first
                    else { continue }
                ...
                case let .discoverCharacteristics(identifier, service):
                    guard peripheral?.identifier.uuidString == identifier,
                        let gattService = peripheral?.services?.first(where: { $0.uuid == service.coreBluetoothUuid })
                    else { continue }
                ...
                case let .write(identifier, write):
                    guard let targetCharacteristic = melkCharacteristic(identifier, uuid: write.characteristic),
                        targetCharacteristic.properties.contains(.writeWithoutResponse)
                    else { continue }
        ...
        private func emitIdentity(rssi: Int? = nil) {
            ...
            onIdentity?(
                MelkLightingPeripheralIdentity(
                    name: peripheralName, platformIdentifier: peripheralIdentifier, rssi: rssi))
        }
        ...
        private static func state(_ dto: MobileMelkLightingSessionStateDto) -> MelkLightingPeripheralState {
            ...
            case let .retrying(attempt, delayMilliseconds):
                .retrying(attempt: Int(attempt), delayMilliseconds: delayMilliseconds)
