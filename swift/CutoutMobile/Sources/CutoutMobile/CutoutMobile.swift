...
public enum CutoutSessionError: Error, Equatable, Sendable {
    case commandRefused(CommandRefusalReason?)
...
public final class VescOnewheelSession: @unchecked Sendable {
    ...
    fileprivate func step(
        ...
        let actions = result.outputs.map(SessionAction.init)
        if kind == .linkDown {
...
public struct CoreBluetoothTransportPlanner: Equatable, Hashable, Sendable {
    ...
    public func plan(action: SessionAction) -> [CoreBluetoothPlannedOperation] {
        ...
        case .disconnect:
            return [.disconnect]
        case .event, .notificationIngest, .faultHistoryReadback, .bmsSnapshot:
            return []
        }
    }
...
/// Result of handing a write to the native Bluetooth transport.
public enum CoreBluetoothWriteDisposition: Equatable, Hashable, Sendable {
    /// The peripheral accepted the write immediately.
    case submitted
    /// The host retained the write until CoreBluetooth can accept it.
    case queued
...
public struct CoreBluetoothOperationExecutor {
    ...
    @discardableResult
    public func execute(_ operation: CoreBluetoothPlannedOperation) -> CoreBluetoothWriteDisposition? {
        switch operation {
        case .subscribe(let channel):
            sink?.subscribe(channel: channel)
            return nil
        case .writeWithoutResponse(let channel, let bytes):
            return sink?.writeWithoutResponse(channel: channel, bytes: bytes) ?? .rejected
        case .disconnect:
            sink?.disconnect()
            return nil
        }
