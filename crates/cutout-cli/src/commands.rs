...
use cutout_core::{
    ...
    CatalogModelResolution, CommandKind, DeviceActionId, DeviceActionRequest, DeviceActionStep,
    DeviceCommand, DeviceEvent, DeviceSettingValue, DiagnosticError, DiagnosticErrorKind,
    DiagnosticSnapshot, FirmwareInfo, GattChannel, GattFingerprint, HostSession, LightState,
    Measured, ModelCatalog, MonotonicTimestamp, NotificationByteLen, ParserDiagnostics,
    PevcapCapture, PevcapDirection, PevcapEncoding, PevcapHeader, PevcapReader, PevcapRecord,
    PevcapReplayMode, PevcapReplayStats, PevcapResolvedIdentity, ProtocolFamily, ProtocolSession,
    ReadOnlyResponse, ReplayChunkComparison, RideOperatingState, SessionInput, SessionKey,
    SessionOutput, SettingId, SettingsReadback, SettingsReadbackAvailability, TelemetrySnapshot,
...
use cutout_protocols::{
    ...
    BEGODE_FALCON_REGISTRY_ENTRY, BEGODE_FALCON_SESSION_KEY, BegodeCapacityEvidence,
    BegodeCapacitySelection, BegodePackEvidenceConsistency, BegodePackLayoutEvidence,
    BegodePackLayoutSelection, BegodeVoltageProfileSelection, MODEL_CATALOG,
    ...
    VescStatsMask, encode_refloat_request, find_session_registration,
...
fn falcon_replay_session_from_header(header: &PevcapHeader) -> Result<RegisteredEucSession> {
    match select_falcon_replay_voltage_profile_from_header(header) {
        BegodeVoltageProfileSelection::Selected(_) | BegodeVoltageProfileSelection::Missing => {
            Ok(session_registration_by_key(BEGODE_FALCON_SESSION_KEY)?.construct())
        }
...
fn select_falcon_replay_voltage_profile_from_header(
    ...
    // The model owns configuration. Captured annotations can contradict it,
    // but neither missing annotations nor placeholder BMS frames change it.
    let configured = cutout_protocols::BegodeFalconModel::PACK_VOLTAGE_PROFILE;
    match select_begode_pack_voltage_profile_from_annotations(header.annotations.iter()) {
        BegodeVoltageProfileSelection::Missing => {
            BegodeVoltageProfileSelection::Selected(configured)
        }
        BegodeVoltageProfileSelection::Selected(profile) if profile == configured => {
            BegodeVoltageProfileSelection::Selected(configured)
        }
        _ => BegodeVoltageProfileSelection::Conflicting,
    }
}
...
fn aero_setting_readback_matches(
    ...
    let (field_id, expected_value) = match command {
        DeviceCommand::SetSetting { id, value } => match (id, value) {
            (SettingId::SpeedAlarmThreshold, DeviceSettingValue::Number(value)) => {
                (VETERAN_FIELD_SPEED_ALERT_DECI_KMH, i64::from(value))
            }
            (SettingId::TiltbackSpeed, DeviceSettingValue::Number(value)) => {
                (VETERAN_FIELD_SPEED_TILTBACK_DECI_KMH, i64::from(value))
            }
            (SettingId::PwmTiltback, DeviceSettingValue::Disabled) => (AERO_FIELD_PWM_PERCENT, 200),
            (SettingId::PwmTiltback, DeviceSettingValue::Number(value)) => {
                (AERO_FIELD_PWM_PERCENT, i64::from(value))
            }
            ...
            (SettingId::PedalHardness, DeviceSettingValue::Number(value)) => {
                (AERO_FIELD_PEDAL_HARDNESS_PERCENT, i64::from(value))
            }
            (SettingId::DisplayBrightness, DeviceSettingValue::Number(value)) => {
                (AERO_FIELD_DISPLAY_BACKLIGHT_PERCENT, i64::from(value))
            }
            (SettingId::BeeperVolumePercent, DeviceSettingValue::Number(value)) => {
                (AERO_FIELD_BEEPER_VOLUME_PERCENT, i64::from(value))
            }
            (SettingId::DynamicAssist, DeviceSettingValue::Number(value)) => {
                (AERO_FIELD_DYNAMIC_ASSIST_PERCENT, i64::from(value))
            }
            (SettingId::PedalDipCompensation, DeviceSettingValue::Number(value)) => {
                (AERO_FIELD_PEDAL_DIP_COMPENSATION_PERCENT, i64::from(value))
            }
            (SettingId::LateralTiltLimit, DeviceSettingValue::Number(value)) => {
                (AERO_FIELD_LATERAL_TILT_LIMIT_DEGREES, i64::from(value))
            }
            ...
            (SettingId::ChargeLimitDiagnostic, DeviceSettingValue::Number(value)) => {
                (AERO_FIELD_MAX_CHARGE_VOLTAGE_RAW, i64::from(value))
            }
            (SettingId::DisplayUnits, DeviceSettingValue::Choice(value)) => {
                (AERO_FIELD_WHEEL_UNITS, i64::from(value))
            }
            (SettingId::HighSpeedMode, DeviceSettingValue::Boolean(value)) => {
                (AERO_FIELD_HIGH_SPEED_MODE, i64::from(u8::from(value)))
            }
            (SettingId::LowBatteryMode, DeviceSettingValue::Boolean(value)) => {
                (AERO_FIELD_LOW_BATTERY_MODE, i64::from(u8::from(value)))
            }
            (SettingId::TransportMode, DeviceSettingValue::Boolean(value)) => {
                (AERO_FIELD_TRANSPORT_MODE, i64::from(u8::from(value)))
            }
            _ => return None,
        },
        _ => return None,
...
fn parse_aero_write_command(setting: AeroSetting, value: &str) -> Result<DeviceCommand> {
    match setting {
        ...
        AeroSetting::BrakeOverpressureAlarm => semantic_range(
            SettingId::BrakeOverpressureAlarm,
            value,
            90..=125,
            "brake overpressure alarm must be 90..=125 percent",
        ),
        AeroSetting::DisplayBacklight => semantic_range(
            SettingId::DisplayBrightness,
            value,
            0..=100,
            "display backlight must be 0..=100 percent",
        ),
        AeroSetting::BeeperVolume => semantic_range(
            SettingId::BeeperVolumePercent,
            value,
            0..=100,
            "beeper volume must be 0..=100 percent",
        ),
        AeroSetting::DynamicAssist => semantic_range(
            SettingId::DynamicAssist,
            value,
            0..=100,
            "dynamic assist must be 0..=100 percent",
        ),
        AeroSetting::PedalDipCompensation => semantic_range(
            SettingId::PedalDipCompensation,
            value,
            0..=100,
            "pedal dip compensation must be 0..=100 percent",
        ),
        AeroSetting::LateralTiltLimit => semantic_range(
            SettingId::LateralTiltLimit,
            value,
            35..=75,
            "lateral tilt limit must be 35..=75 degrees",
        ),
        AeroSetting::VoltageCorrection => semantic_range(
            SettingId::VoltageCorrection,
            value,
            -15..=15,
            "voltage correction must be -15..=15 tenths of a percent",
        ),
...
fn semantic_range(
    ...
    let value = value.parse::<i32>().context(message)?;
...
fn semantic_speed(id: SettingId, value: &str, label: &str) -> Result<DeviceCommand> {
    let speed = value
        .parse::<i32>()
        .with_context(|| format!("{label} speed must be 10..=200 km/h"))?;
...
fn semantic_choice(id: SettingId, value: &str, label: &str) -> Result<DeviceCommand> {
    let choice = if label == "wheel units" {
        match value {
            "metric" => 0,
            "imperial" => 1,
...
fn vesc_probe_sequence(probes: &[VescProbe]) -> Vec<VescProbe> {
    ...
    let mut sequence = Vec::with_capacity(probes.len() + 1);
    let mut has_refloat_ids = false;
    for probe in probes.iter().copied() {
        if probe == VescProbe::RefloatRealtime && !has_refloat_ids {
            sequence.push(VescProbe::RefloatRealtimeIds);
            has_refloat_ids = true;
        }
        if probe == VescProbe::RefloatRealtimeIds {
            has_refloat_ids = true;
        }
        sequence.push(probe);
    }
...
fn append_falcon_capture_resolver_context(
    ...
    let voltage_profile =
        select_begode_pack_voltage_profile_from_annotations(annotations.iter().copied());
...
fn encode_session_capture_pevcap(
    ...
    if profile.is_falcon() {
        append_falcon_capture_resolver_context(annotations, &mut resolver_warnings);
    }
...
fn print_vesc_reply_summary(reply: &VescReadOnlyReply) {
    match reply {
        ...
        VescReadOnlyReply::MotorConfig(config) => info!(
            motor_pole_pairs = config.speed_geometry.motor_pole_pairs.get(),
            gear_ratio_denominator = config.speed_geometry.gear_ratio_denominator.get(),
            wheel_circumference_mm = config.speed_geometry.wheel_circumference.as_millimetres(),
            battery_type = ?config.battery_type,
            battery_cells = config.battery_cells.get(),
            "vesc motor config"
        ),
        VescReadOnlyReply::MotorSetupConfig(geometry) => info!(
            motor_pole_pairs = geometry.motor_pole_pairs.get(),
            gear_ratio_denominator = geometry.gear_ratio_denominator.get(),
            wheel_circumference_mm = geometry.wheel_circumference.as_millimetres(),
            "vesc motor setup config"
        ),
    }
}
...
fn vesc_reply_json(reply: &VescReadOnlyReply) -> serde_json::Value {
    match reply {
        ...
        VescReadOnlyReply::MotorConfig(config) => serde_json::json!({
            "type": "vesc_motor_config",
            "motor_pole_pairs": config.speed_geometry.motor_pole_pairs.get(),
            "gear_ratio_denominator": config.speed_geometry.gear_ratio_denominator.get(),
            "wheel_circumference_mm": config.speed_geometry.wheel_circumference.as_millimetres(),
            "battery_type": format!("{:?}", config.battery_type),
            "battery_cells": config.battery_cells.get(),
        }),
        VescReadOnlyReply::MotorSetupConfig(geometry) => serde_json::json!({
            "type": "vesc_motor_setup_config",
            "motor_pole_pairs": geometry.motor_pole_pairs.get(),
            "gear_ratio_denominator": geometry.gear_ratio_denominator.get(),
            "wheel_circumference_mm": geometry.wheel_circumference.as_millimetres(),
        }),
...
const fn command_kind_name(kind: CommandKind) -> &'static str {
    match kind {
        ...
        CommandKind::GyroCalibration => "gyro_calibration",
        CommandKind::SetSetting => "set_setting",
...
#[cfg(test)]
mod tests {
    use cutout_protocols::BEGODE_DATA_CHANNEL;
        ...
        for value in ["0", "30", "64", "100"] {
            assert_eq!(
                ...
                DeviceCommand::SetSetting {
                    ...
                    value: DeviceSettingValue::Number(value.parse().unwrap()),
                }
            );
        }
    ...
    #[test]
    fn capture_pevcap_does_not_invent_bms_evidence_from_falcon_placeholders() {
        let summary = falcon_connection_summary();
        let capture = SessionCapture {
    ...
    #[test]
    fn pevcap_replay_uses_model_when_falcon_battery_evidence_is_missing() {
        ...
        let report = replay_pevcap_capture(&capture, profile)
            .expect("model configuration does not require independent voltage evidence");
        assert_eq!(
            select_falcon_replay_voltage_profile(&capture),
            BegodeVoltageProfileSelection::Selected(
                cutout_protocols::BegodeFalconModel::PACK_VOLTAGE_PROFILE
            )
        );
        assert!(report.chunk_one_byte_matches);
        assert!(report.chunk_arbitrary_matches);
    ...
    #[test]
    fn pevcap_replay_uses_explicit_voltage_despite_falcon_bms_placeholders() {
        ...
        let profile = selected_pevcap_replay_profile(&capture, SessionProfile::Auto)
            .expect("Falcon identity selects replay profile");
        let report = replay_pevcap_capture(&capture, profile)
            .expect("explicit annotation selects Falcon 100.8V profile");
    ...
    #[test]
    fn pevcap_replay_ignores_chunked_falcon_bms_placeholders_as_voltage_evidence() {
        let capture = sample_falcon_replay_capture_with_records(
            &[],
    ...
    #[test]
    fn pevcap_replay_ignores_ambiguous_falcon_bms_voltage_evidence() {
        let capture = sample_falcon_replay_capture_with_records(
            &[],
            vec![PevcapRecord::inbound_notification(
                ms(41),
    ...
    fn replay_voltage_selection_rejects_conflicts_instead_of_overriding_evidence() {
        for (annotations, succeeds) in [
            (vec![], true),
            (vec!["battery=100v"], true),
            (vec!["battery=84v"], false),
            (vec!["battery=84v", "battery=100v"], false),
        ] {
            let capture = sample_falcon_live_a_replay_capture(&annotations);
            let result = falcon_replay_session_from_header(&capture.header);
    ...
    fn pevcap_replay_corpus_auto_selects_and_matches_chunk_modes() {
        for case in PEVCAP_REPLAY_CORPUS {
            ...
            // Compare the shared model session against production replay selection.
            let report = replay_pevcap_with_session(
                &capture,
                profile
                    .session_registration()
                    .expect("registered corpus profile")
                    .construct(),
            )
    ...
    #[test]
    fn pevcap_public_replay_uses_model_battery_without_rewriting_capture() {
        use clap::Parser;
        ...
        .expect("model selection is sufficient for replay");
        ...
        pevcap_replay(&auto).expect("resolved model supplies its battery configuration");
    ...
    fn pevcap_replay_falcon_riding_capture_parses_all_events() {
        ...
        let report = replay_pevcap_with_session(
            ...
            profile
                .session_registration()
                .expect("registered Falcon profile")
                .construct(),
        )
        .expect("falcon riding capture should replay with explicit session configuration");

        assert_eq!(profile, selected_falcon_session_profile());
        assert_eq!(report.replay_records.get(), 1153);
        assert_eq!(report.outputs.get(), 2882);
        // 192 each of Live A, Live B and extra telemetry; BMS-shaped frames
        // are raw evidence only on standard Falcon.
        ...
        assert_eq!(
            aero_setting_readback_matches(
                DeviceCommand::SetSetting {
                    ...
                    value: DeviceSettingValue::Number(530),
                },
                &readbacks,
            ),
            Some(true)
        ...
        assert_eq!(
            aero_setting_readback_matches(
                DeviceCommand::SetSetting {
                    ...
                    value: DeviceSettingValue::Number(540),
                },
                &readbacks,
            ),
            Some(false)
    ...
    fn aero_write_parser_exposes_every_source_backed_aero_setting() {
        let cases = [
            (AeroSetting::RidingMode, "hard", CommandKind::SetSetting),
            (AeroSetting::TiltbackSpeed, "200", CommandKind::SetSetting),
            (AeroSetting::Pwm, "off", CommandKind::SetSetting),
            (AeroSetting::AlarmSpeed, "10", CommandKind::SetSetting),
            (
            ...
            (AeroSetting::BeeperVolume, "0", CommandKind::SetSetting),
            (AeroSetting::DynamicAssist, "42", CommandKind::SetSetting),
            (
                AeroSetting::PedalDipCompensation,
                "42",
                CommandKind::SetSetting,
            ),
            (AeroSetting::LateralTiltLimit, "35", CommandKind::SetSetting),
            (
                AeroSetting::VoltageCorrection,
                "-15",
                CommandKind::SetSetting,
            ),
            (AeroSetting::WheelUnits, "imperial", CommandKind::SetSetting),
            (AeroSetting::HighSpeedMode, "on", CommandKind::SetSetting),
            (AeroSetting::LowBatteryMode, "off", CommandKind::SetSetting),
            (AeroSetting::TransportMode, "on", CommandKind::SetSetting),
        ];
        ...
        assert!(parse_aero_write_command(AeroSetting::MaxChargeVoltageRaw, "70").is_err());
        ...
        [
            ...
            (
                DeviceCommand::SetSetting {
                    ...
                    value: DeviceSettingValue::Number(110),
                },
                0x0841,
                110,
            ),
            ...
            (
                DeviceCommand::SetSetting {
                    ...
                    value: DeviceSettingValue::Number(25),
                },
                0x083f,
                25,
            ),
            (
                DeviceCommand::SetSetting {
                    ...
                    value: DeviceSettingValue::Number(42),
                },
                0x0842,
                42,
            ),
            (
                DeviceCommand::SetSetting {
                    ...
                    value: DeviceSettingValue::Number(33),
                },
                0x0844,
                33,
            ),
        ...
        [
            ...
            (
                DeviceCommand::SetSetting {
                    ...
                    value: DeviceSettingValue::Number(-5),
                },
                0x083b,
                -5,
            ),
    ...
    #[test]
    fn vesc_probe_sequence_primes_refloat_realtime_ids() {
        assert_eq!(
            vesc_probe_sequence(&[VescProbe::RefloatRealtime]),
            vec![VescProbe::RefloatRealtimeIds, VescProbe::RefloatRealtime]
        );
        assert_eq!(
            vesc_probe_sequence(&[
                VescProbe::Firmware,
                VescProbe::RefloatRealtimeIds,
    ...
    #[test]
    fn command_kind_names_cover_generic_settings_and_actions() {
