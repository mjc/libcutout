...
| Stationary-only settings writes | `SetPedalMode` (typed; Veteran `SETh`/`SETm`/`SETs`, Begode `h`/`f`/`s`) | `SupportsSettingsWrites` plus a settings-write session shell | Model allow-list, stationary-state evidence where available, explicit command capability, no probing overlap | Encoder and refusal paths tested; live write session not enabled |
