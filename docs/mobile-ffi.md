...
Repository Swift/Xcode scripts verify that the package exists, contains the
required architectures, and was generated from the current Rust inputs before
building. Normal Swift-only work does not rebuild Rust, generate bindings, set
dynamic-library paths, or pass custom linker flags.

## Regenerating the Swift package

Regenerate the package after changing Rust code that ships in the app:
...
the package. All repository Swift/Xcode entry points fail before building when
that fingerprint is missing or stale.
...
No preparation step is required while the checked-in artifact fingerprint
matches the Rust sources. After a Rust input changes, regenerate the package
before activating SourceKit or building Swift.
