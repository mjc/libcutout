...
import Foundation
    ...
    public final class SpotifyProviderAdapter: NSObject {
        ...
        private final class AppRemoteBridge: NSObject, SPTAppRemoteDelegate, SPTAppRemotePlayerStateDelegate {
            ...
            init(
                owner: SpotifyProviderAdapter, providerGeneration: MobileMusicProviderSessionId,
        ...
        private var artworkRequest:
            (id: MobileMusicArtworkRequestId, trackURI: String, generation: MobileMusicProviderSessionId)?
        ...
        public func refreshPlayerState() {
            ...
            guard let playerAPI = appRemote?.playerAPI,
                let bridge = appRemoteBridge,
                let requestID = lifecycle.beginPlayerStateRequest(nowMs: nowMs)
            else { return }
        ...
        private func finishAuthorizationTransaction(generation: MobileMusicAuthorizationId)
        ...
        private func detachAppRemote(
            providerGeneration: MobileMusicProviderSessionId, attemptID: MobileMusicConnectionAttemptId
        ) {
        ...
        private func handlePlayerStateDidChange(
            ...
            guard onChange != nil,
                appRemote != nil,
                lifecycle.classifyProviderSession(id: providerGeneration) == .current,
                lifecycle.classifyConnection(id: attemptID, nowMs: connectionNowMs) == .accepted
            else { return }
            let trackChanged = self.playerState?.track.uri != playerState.track.uri
            #if DEBUG
                if trackChanged {
                    print(
                        "spotify_player_state uri_bytes=\(playerState.track.uri.utf8.count) title_bytes=\(playerState.track.name.utf8.count) artist_bytes=\(playerState.track.artist.name.utf8.count) position=\(playerState.playbackPosition) duration=\(playerState.track.duration)"
                    )
                }
            #endif
            ...
            imageAPI.fetchImage(forItem: track, with: Self.artworkSize) { [weak self] image, error in
                ...
                Task { @MainActor [weak self, image, failed] in
                    ...
                    guard let self,
                        ...
                        self.playerState?.track.uri == trackURI
                    else { return }
                    guard
        ...
        private func scheduleArtworkRetry(
            ...
            guard
                let effect = lifecycle.beginArtworkRetryEffect(
                    providerGeneration: generation,
                    nowMs: connectionNowMs
                )
            else { return }
            ...
            effects.run(
                ...
                guard let self,
                    ...
                    self.lifecycle.classifyProviderSession(id: generation) == .current,
                    self.playerState?.track.uri == track.uri
                else { return }
        ...
        private func emitChange() {
                ...
                let diagnostic =
                    "spotify_observation state=\(lifecycleState) has_item=\(snapshot.item != nil) monitoring=\(onChange != nil)"
