...
struct CutoutMobilePackageSmoke {
    static func main() throws {
        ...
        precondition(LiveRideConnectionPhase.starting.displayText == "Starting Bluetooth...")
        precondition(LiveRideConnectionPhase.scanning(model: .aero).displayText == "Scanning for Aero...")
        precondition(LiveRideConnectionPhase.live.displayText == "Live")
        precondition(LiveRideConnectionPhase.bluetoothUnavailable(rawState: 4).displayText == "Bluetooth unavailable: state 4")
        precondition(LiveRideConnectionPhase.failed(.missingNotifyChannel).displayText == "Missing notify channel")
        ...
        let initialSpeedState = LiveRideDisplayState()
