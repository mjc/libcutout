...
import Foundation
    ...
    struct MelkLightingLiveValidator {
        static func main() {
            ...
            session.onCandidate = { candidate in
                print(
                    "candidate name=\(candidate.name ?? "unknown") id=\(candidate.id) rssi=\(candidate.rssi); enter `select \(candidate.id)`"
                )
            }

            session.onIdentity = { identity in
                print(
                    "identity name=\(identity.name ?? "unknown") id=\(identity.platformIdentifier) rssi=\(identity.rssi.map(String.init) ?? "unknown")"
                )
            }
        ...
        private static func handle(
            ...
            case "help":
                print(
                    "select <UUID>; retry; power on|off; color R G B; brightness 0-100; speed 0-255; effect 0-227 [speed]; confirm; unconfirm; quit"
                )
        ...
        private static func handleLighting(_ parts: [String], session: MelkLightingPeripheralSession) {
            switch parts[0] {
            case "color":
                guard let r = UInt8(parts[1]), let g = UInt8(parts[2]), let b = UInt8(parts[3]) else {
                    print("invalid color")
                    return
                }
                print("requested=\(session.setSolidColor(red: r, green: g, blue: b))")
            case "brightness":
                guard let value = UInt8(parts[1]) else {
            ...
            case "effect":
                ...
                let state = MobileMelkLightingRestoreStateDto(
                    powerOn: true, red: 255, green: 255, blue: 255, brightness: 100,
                    playback: .effect(pattern: pattern, speed: speed))
    ...
    struct MelkLightingLiveValidator {
        static func main() {
            print("CoreBluetooth is unavailable on this platform")
            exit(EXIT_FAILURE)
        }
    }
#endif
