...
use cutout_core::{
    Capabilities, CommandKind, DeviceCommand, DeviceEvent, GattChannel, ModelRegistryEntry,
    MonotonicTimestamp, NotificationByteLen, NotificationIngestOutcome, ParserDiagnostics,
    ParserError, ProtocolFamily, ProtocolSession, SafetyClass, SessionInput, SessionOutput,
    SessionOutputError, SessionOutputSink, TransportAction, WritePayload,
};
...
pub(crate) fn push_parser_error(
    error: ParserError,
    output: &mut dyn SessionOutputSink,
) -> Result<(), SessionOutputError> {
...
mod tests {
    ...
    use super::*;
    use crate::{
        AeroProbe, AeroRequestEncoder, BEGODE_DATA_CHANNEL, BegodeFalconModel,
        BegodeNotificationDecoder, BegodePackVoltageProfile, GearRatioDenominator, MotorPolePairs,
        NosfetAeroModel, RequestDisposition, VESC_NOTIFY_CHANNEL, VESC_RAW_CONTROLLER_ID_FIELD_ID,
        VESC_RAW_ERPM_FIELD_ID, VESC_RAW_FAULT_CODE_FIELD_ID, VESC_RAW_STATS_COUNT_TIME_FIELD_ID,
        VESC_RAW_STATS_CURRENT_AVG_FIELD_ID, VESC_RAW_STATS_POWER_AVG_FIELD_ID,
        VESC_RAW_STATS_SPEED_AVG_FIELD_ID, VESC_RAW_TACHOMETER_FIELD_ID, VESC_WRITE_CHANNEL,
        VETERAN_DATA_CHANNEL, VescBoardProfile, VescGenericModel, VescNotificationDecoder,
        VeteranBmsMetadataPage, VeteranBmsPageEvidence, VeteranFrameReassembler,
        VeteranNotificationDecoder, begode_falcon_target_voltage_profile,
        veteran_session::veteran_bms_metadata_payload,
    };
    use core::mem::size_of;
    use cutout_core::{
        BatteryCurrent, BatteryPageKind, LinkInfo, Measured, ParserGapEvidence, PayloadBodyLen,
        PayloadClassifier, ProtocolSelector, RawFieldValue, ReadOnlyResponse,
        ReservedPayloadEvidence, SemanticEventCount, TelemetryDelta, Temperature, TransportAction,
