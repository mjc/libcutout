...
| Detection | `identification.rs` now suppresses Begode probes once Veteran/VESC/conflict evidence resolves the protocol and retires their pending Rust state; mobile admission promotes an exact supported Rust candidate before any family-specific fallback probe. Fragmented evidence remains pending, while the historical NF2557 run sent Begode N/V/M probes after a Veteran model frame had already identified the Aero | Shared GATT characteristics cannot by themselves establish that a probe is harmless. Native queued-write cancellation and safe probing while multiple protocols remain plausible are still open. Capture is a terminal unresolved path, not the immediate response to incomplete data. |
