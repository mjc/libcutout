...
final class CutoutAppModelTests: XCTestCase {
    ...
    @MainActor
    func testMusicHistoryPolicySelectedBeforeRideAppliesWhenRideStarts() throws {
        let driver = SessionDriverSpy(rows: [])
        let model = CutoutAppModel(core: driver)
        ...
        let outcome = try driver.rideMapStateHandle.recordMusicEvent(
    ...
    func testForgetMusicHistoryPreservesTheActiveRideAndClearsItsEvents() throws {
        ...
        XCTAssertTrue(model.startGpsOnlyRide())
        XCTAssertTrue(model.setMusicHistoryPolicy(.opaqueItem))
        _ = try driver.rideMapStateHandle.recordMusicEvent(
            snapshot: MobileMusicSnapshotDto(
                provider: .appleMusic,
                sessionId: "session-1",
                state: .playing,
        ...
        let rideID = driver.rideMapStateHandle.currentSnapshot()!.rideId

        XCTAssertTrue(model.deleteMusicHistory(rideID: rideID))
        XCTAssertTrue(model.isRideMapRecording)
        XCTAssertTrue(driver.rideMapStateHandle.currentMusicEvents()!.isEmpty)
    }

    @MainActor
    func testDiscardedRideClearsMusicCaptureContextWithoutChangingFuturePolicy() {
        let driver = SessionDriverSpy(rows: [])
        let model = CutoutAppModel(core: driver)
        XCTAssertTrue(model.setMusicHistoryPolicy(.opaqueItem))
        XCTAssertTrue(model.startGpsOnlyRide())
    ...
    @MainActor
    func testSavedRideClearsMusicCaptureContextWithoutChangingFuturePolicy() {
        let driver = SessionDriverSpy(rows: [])
        let model = CutoutAppModel(core: driver)
        XCTAssertTrue(model.setMusicHistoryPolicy(.opaqueItem))
        XCTAssertTrue(model.startGpsOnlyRide())
    ...
    @MainActor
    func testForgetMusicHistoryDisablesFutureCaptureAndClearsCurrentContext() {
        let driver = SessionDriverSpy(rows: [])
        let model = CutoutAppModel(core: driver)
        XCTAssertTrue(model.startGpsOnlyRide())
        XCTAssertTrue(model.setMusicHistoryPolicy(.opaqueItem))
